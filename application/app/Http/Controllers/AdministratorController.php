...
class AdministratorController extends Controller
{
    ...
    public function addAdministrator(Request $request)
    {
        ...
        $v = \Validator::make($request->all(), [
            'first_name' => 'required', 'username' => 'required', 'email' => 'required', 'password' => 'required', 'cpassword' => 'required', 'role' => 'required'
        ]);

        if ($v->fails()) {
            return redirect('administrators/all')->withErrors($v->errors());
        }
    ...
    public function postUpdateAdministrator(Request $request)
    {
        ...
        $v = \Validator::make($request->all(), [
            'first_name' => 'required', 'username' => 'required', 'email' => 'required', 'role' => 'required', 'status' => 'required'
        ]);
        ...
        if ($v->fails()) {
            return redirect('administrators/manage/' . $cmd)->withErrors($v->errors());
        }
    ...
    public function addAdministratorRole(Request $request)
    {
        ...
        if ($v->fails()) {
            return redirect('administrators/role')->withErrors($v->errors());
        }

        $exist = AdminRole::where('role_name', $request->role_name)->first();
        if ($exist) {
            return redirect('administrators/role')->with(['message' => 'Administrator Role already exist', 'message_important' => true]);
        }
    ...
    public function updateAdministratorSetRole(Request $request)
    {
        ...
        if ($v->fails()) {
            return redirect('administrators/set-role/' . $role_id)->withErrors($v->errors());
        }
