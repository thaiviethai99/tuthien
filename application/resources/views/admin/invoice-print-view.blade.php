...
<head>
    ...
    <title>{{language_data('Invoice')}} -{{$inv->id}}</title>
...
<div id="page-wrap">
    ...
    <table width="100%">
        <tr>
            <td style="border: 0;  text-align: left" width="62%">
                <span style="font-size: 18px; color: #2f4f4f"><strong>{{language_data('Invoice')}} # {{$inv->id}}</strong></span>
            </td>
    ...
    <div id="customer">
        ...
        <table id="meta">
            <tr>
                ...
                <td class="meta-head">{{language_data('Invoice')}} #</td>
                <td>{{$inv->id}}</td>
            </tr>
            <tr>

                <td class="meta-head">{{language_data('Status')}}</td>
                <td>{{get_date_format($inv->status)}}</td>
            </tr>
            <tr>

                <td class="meta-head">{{language_data('Invoice Date')}}</td>
                <td>{{get_date_format($inv->created)}}</td>
            </tr>
            <tr>

                <td class="meta-head">{{language_data('Due Date')}}</td>
                <td>{{get_date_format($inv->duedate)}}</td>
            </tr>

            <tr>

                <td class="meta-head">{{language_data('Amount Due')}}</td>
                <td>
    ...
    <table id="items">
        ...
        <tr>
            <th width="65%">{{language_data('Item')}}</th>
            <th align="right">{{language_data('Price')}}</th>
            <th align="right">{{language_data('Quantity')}}</th>
            <th align="right">{{language_data('Total')}}</th>
        ...
        @if($tax_sum!='0.00' OR $tax_sum!='')
            <tr>
                <td class="blank"></td>
                <td colspan="2" class="total-line">{{language_data('Subtotal')}}</td>
            ...
            <tr>
                ...
                <td colspan="2" class="total-line">{{language_data('Tax')}}</td>
                <td class="total-value">
        ...
        @if($dis_sum!='0.00' OR $dis_sum!='')
            <tr>
                <td class="blank"></td>
                <td colspan="2" class="total-line">{{language_data('Discount')}}</td>
                <td class="total-value">
        ...
        <tr>
            <td class="blank"></td>
            <td colspan="2" class="total-line balance">{{language_data('Grand Total')}}</td>
