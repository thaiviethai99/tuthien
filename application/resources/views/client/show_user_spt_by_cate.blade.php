...
@section('content')
    ...
    <section class="wrapper-bottom-sec">
        <div class="p-30">
            <h2 class="page-title">
               <?php
                                                $cate=DB::selectOne('select * from room c where c.id='.Request::route('cateIdSub'));
                                                echo $cate->name;
                                            ?>
            </h2>
        </div>
                            ...
                            <table class="table data-table table-hover table-ultra-responsive">
                                ...
                                @foreach($clients as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-success btn-xs" href="{{url('client/edit-user-spt/'.$item->id.'/'.Request::route('cateIdSub'))}}"><i class="fa fa-edit"></i>Sửa</a>
...
<script>
    $( "body" ).delegate( ".cdelete", "click",function (e) {
                ...
                bootbox.confirm("Are you sure?", function (result) {
                    if (result) {
                        var _url = $("#_url").val();
                        window.location.href = _url + "/client/delete-user-spt-in-cate/" + id +'/'+{{$cate->id}};
                    }
                });
            });
</script>
